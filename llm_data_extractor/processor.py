...
# %% auto 0
__all__ = ['logger', 'REQUIRED_COLUMNS', 'format_for_db', 'format_for_target_tables', 'create_summary_stats',
           'create_audit_record', 'process_query']
...
def _process_batch(batch_info: Dict[str, Any], llm_config: LLMConfig) -> List[ExtractionResult]:
    """Helper function to process a single batch in a separate thread."""
    results, metadata = extract_data(
    ...
    # Attach batch-specific info to each result
    for r in results:
...
def process_query(query: str, llm_config: LLMConfig, db_config: DBConfig, results_table_name: str, max_workers: int = 4) -> Dict[str, Any]:
    ...
    def parse_config(x):
        ...
        return {}
    print('Group data into batches')
    df['answer_config'] = df['answer_config'].apply(parse_config)

    # Now this prints a dict already
    print(df['answer_config'].iloc[0])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_batch):
            try:
                ...
                print(f"Batch for input_row_id {batch_info['input_row_id']} generated an exception: {exc}")
